...
const SchoolAdmissionTable = ({
  ...
  useEffect(() => {
    if (schoolAdmissionFormData && schoolAdmissionFormData.length > 0) {
      setFilteredData(schoolAdmissionFormData);
    }
  }, [schoolAdmissionFormData]);
  const locations = useMemo(() => {
    return Array.from(
      new Set(schoolAdmissionFormData?.map((job) => job.State?.name || ""))
    );
  }, [schoolAdmissionFormData]);

  const categories = useMemo(() => {
    return Array.from(
      new Set(schoolAdmissionFormData?.map((job) => job.Category?.name || ""))
    );
  }, [schoolAdmissionFormData]);
  const departments = useMemo(() => {
    return Array.from(
      new Set(schoolAdmissionFormData?.map((job) => job.Depertment?.name || ""))
    );
  }, [schoolAdmissionFormData]);

  const contentData = useMemo(() => {
    return schoolAdmissionFormData?.map((contentItem) =>
  ...
  const handleSearch = useCallback(
    (term) => {
      ...
      // Filter the data
      const filtered = schoolAdmissionFormData.filter((formData) =>
        searchTerms.some(
          (searchTerm) =>
  ...
  const handleFilter = (filters) => {
    const filtered = schoolAdmissionFormData.filter((job) => {
...
export default SchoolAdmissionTable;
