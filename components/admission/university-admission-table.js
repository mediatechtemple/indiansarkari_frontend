...
const UniversityAdmissionTable = ({
  universityAdmissionFormData = [],
  headers = [],
}) => {
  ...
  useEffect(() => {
    if (universityAdmissionFormData && universityAdmissionFormData.length > 0) {
      setFilteredData(universityAdmissionFormData);
    }
  }, [universityAdmissionFormData]);
  const locations = useMemo(() => {
    return Array.from(
      new Set(universityAdmissionFormData?.map((job) => job.State?.name || ""))
    );
  }, [universityAdmissionFormData]);
  ...
  const categories = useMemo(() => {
    return Array.from(
      new Set(
        universityAdmissionFormData?.map((job) => job.Category?.name || "")
      )
    );
  }, [universityAdmissionFormData]);
  const departments = useMemo(() => {
    return Array.from(
      new Set(
        universityAdmissionFormData?.map((job) => job.Depertment?.name || "")
      )
    );
  }, [universityAdmissionFormData]);

  const contentData = useMemo(() => {
    return universityAdmissionFormData?.map((contentItem) =>
  ...
  const handleSearch = useCallback(
    (term) => {
      ...
      // Filter the data
      const filtered = universityAdmissionFormData.filter((formData) =>
        searchTerms.some(
          (searchTerm) =>
  ...
  const handleFilter = (filters) => {
    const filtered = universityAdmissionFormData.filter((job) => {
          ...
          {filteredData &&
            ...
            filteredData.map((item) => (
              ...
              >
                ...
                <TableCell className="border border-white px-4 py-1">
                  {new Date(item?.admission?.created_at).toLocaleDateString(
                    "en-GB"
                  )}
                </TableCell>
...
export default UniversityAdmissionTable;
