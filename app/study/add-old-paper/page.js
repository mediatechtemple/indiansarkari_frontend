...
import { initialOldPaperFormData, postData } from "@/utils";
import { RiCloseLine } from "@remixicon/react";
import { useRouter } from "next/navigation";
import Loading from "@/app/loading";
import { oldPaperPostFormControlls } from "@/config";

const AddOldPaperPost = () => {
  const [formData, setFormData] = useState(initialOldPaperFormData);
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      if (data) {
        setFormData(initialOldPaperFormData);
    ...
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <Card className="min-w-full shadow-lg">
        ...
        <CardContent className="p-5">
          ...
          {/* Display error */}
          <CommonForm
            formControlls={oldPaperPostFormControlls}
...
export default AddOldPaperPost;
